...
interface OngoingElection {
  ...
  liveVotingRate: number;
}

// ===== MOCK DATA =====
const MOCK_ELECTIONS: OngoingElection[] = [
  {
    id: "mock-1",
...
const OngoingElections: React.FC = () => {
  const { elections: supabaseElections, loading } = useElections();
  const navigate = useNavigate();
  const [searchQuery, setSearchQuery] = useState("");
  const [filterStatus, setFilterStatus] = useState<"all" | "active" | "paused">("all");

  // ===== CONVERT SUPABASE ELECTIONS TO MATCH MOCK FORMAT =====
  const convertedSupabaseElections: OngoingElection[] = supabaseElections
    .filter((election) => election.status === "active")
    .map((election) => ({
      id: election.id,
      title: election.title,
      description: election.description,
      startDate: election.voting_start_date,
      endDate: election.voting_end_date,
      totalVoters: election.positions?.length * 100 || 0, // Placeholder
      votedCount: 0, // Placeholder - will calculate from votes table later
      positions: election.positions?.length || 0,
      fraudAlerts: 0, // Placeholder - will add fraud detection later
      status: "active" as const,
      liveVotingRate: 0, // Placeholder - will calculate later
    }));

  // ===== COMBINE SUPABASE + MOCK DATA =====
  const elections: OngoingElection[] = [...convertedSupabaseElections, ...MOCK_ELECTIONS];
...
//   // ===== FILTER TO GET ONLY ONGOING ELECTIONS FROM SUPABASE =====
//   const ongoingSupabaseElections = getOngoingElections(supabaseElections);

//   // ===== CONVERT SUPABASE ELECTIONS TO MATCH DISPLAY FORMAT =====
//   const convertedSupabaseElections: OngoingElection[] = ongoingSupabaseElections.map((election) => ({
//     id: election.id,
//     title: election.title,
//     description: election.description,
//     startDate: election.voting_start_date,
//     endDate: election.voting_end_date,
//     totalVoters: election.positions?.length * 100 || 0, // Placeholder - replace with actual voter data
//     votedCount: 0, // Placeholder - will calculate from votes table later
//     positions: election.positions?.length || 0,
//     fraudAlerts: 0, // Placeholder - will add fraud detection later
//     status: election.status === "paused" ? "paused" : "active",
//     liveVotingRate: 0, // Placeholder - will calculate later
//   }));

//   // ===== SORT ELECTIONS (ending soonest first) =====
//   const sortedElections = sortElectionsByDate(
//     convertedSupabaseElections.map(e => ({
//       ...e,
//       voting_start_date: e.startDate,
//       voting_end_date: e.endDate,
...
//   // Convert back to OngoingElection format
//   const elections: OngoingElection[] = sortedElections.map(e => ({
//     id: e.id,
//     title: e.title,
//     description: e.description,
//     startDate: e.voting_start_date,
//     endDate: e.voting_end_date,
//     totalVoters: convertedSupabaseElections.find(ce => ce.id === e.id)?.totalVoters || 0,
//     votedCount: convertedSupabaseElections.find(ce => ce.id === e.id)?.votedCount || 0,
//     positions: convertedSupabaseElections.find(ce => ce.id === e.id)?.positions || 0,
//     fraudAlerts: convertedSupabaseElections.find(ce => ce.id === e.id)?.fraudAlerts || 0,
//     status: e.status === "paused" ? "paused" : "active",
//     liveVotingRate: convertedSupabaseElections.find(ce => ce.id === e.id)?.liveVotingRate || 0,
//   }));

//   // ===== FILTER ELECTIONS BY SEARCH AND STATUS =====
//   const filteredElections = elections.filter((election) => {
//     const matchesSearch =
//       election.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
//       election.description.toLowerCase().includes(searchQuery.toLowerCase());
...
//   // ===== TIME REMAINING (using helper) =====
//   const getTimeRemaining = (endDate: string): string => {
//     const timeInfo = getElectionTimeInfo({
//       id: '',
//       title: '',
//       description: '',
...
//                 ? "Try adjusting your search or filters" 
//                 : "There are no elections currently running"}
//             </p>
//           </div>
//         ) : (
//           <div className="space-y-6">
